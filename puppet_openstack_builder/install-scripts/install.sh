...
if grep 127.0.1.1 /etc/hosts ; then
  sed -i -e "s/127.0.1.1.*/127.0.1.1 $(hostname) $(hostname)/" /etc/hosts
else
...
if ${master}; then
  ...
  export default_interface="${default_interface:-eth4}"
  ...
  export external_interface="${external_interface:-eth5}"
...
git clone -b proxy_start_fix https://github.com/cbaesema/puppet-openstack-ha /usr/share/puppet/modules/openstack-ha
